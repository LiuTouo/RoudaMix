//! Telemetry SHM 讀取端 — 契約:contracts/telemetry_abi.md。
//! `Local\roudamix-telemetry`,seqlock 讀(odd=寫入中),45Hz 輪詢 → emit "meters"。
//! 結構必須與 engine/src/telemetry.hpp TelemetryBlockShm(#pragma pack(8), 560B)同構。
...
const TELEMETRY_NAME: &str = "Local\0roudamix-telemetry";
...
/// 背景 45Hz 輪詢。SHM 出現(= engine 至少 start 過一次)即訂閱,終身持有。
...
const _: () = {
    ...
    assert!(std::mem::size_of::<BlockShm>() == 560);
