...
PdcPlanResult plan_plugin_delay(const std::vector<PdcNodeSpec>& nodes,
    ...
    std::vector<std::size_t> full_pdc_seeds;
    for (const auto& output : outputs)
        if (output.policy == OutputLatencyPolicy::kFullPdc)
            full_pdc_seeds.push_back(g.index.at(output.track_id));
    const std::vector<bool> reaches_full_pdc = graph::reverse_reach(g, full_pdc_seeds);
    std::vector<std::uint64_t> primary_path_latency(nodes.size(), 0);
    std::vector<std::uint64_t> monitor_path_latency(nodes.size(), 0);
    std::vector<std::uint64_t> compensation_path(nodes.size(), 0);
    for (const auto node_index : g.order) {
    ...
    for (const auto& output : outputs) {
        const auto found = g.index.find(output.track_id);
        if (found == g.index.end()) continue;
