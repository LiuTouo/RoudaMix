// Telemetry SHM(`Local\roudamix-telemetry`)— 契約:contracts/telemetry_abi.md(v4)
...
constexpr std::uint32_t kTelemetryAbiVersion = 4;
...
struct PluginLoadShm {
    std::uint32_t instance_id;
    std::uint32_t variant;  // 0 = primary、1 = Monitor Shadow
    float process_load;     // 單一 logical CPU 佔比
    float reserved;
};
static_assert(sizeof(PluginLoadShm) == 16);
...
struct TelemetryBlockShm {
    ...
    std::uint32_t plugin_load_count;
    std::uint32_t plugin_load_reserved;
    PluginLoadShm plugin_loads[kPluginLoadEntries];
};
static_assert(sizeof(TelemetryBlockShm) == 7232);
...
public:
    ...
    void add_plugin_cycles(std::size_t entry, std::uint64_t cycles) noexcept {
        if (entry < kPluginLoadEntries)
            plugin_cycles_[entry].fetch_add(cycles, std::memory_order_relaxed);
    }
...
private:
    ...
    std::atomic<std::uint64_t> plugin_cycles_[kPluginLoadEntries]{};
    // publish thread 專屬(publish 單一 consumer,不必 atomic)
    std::uint64_t last_tsc_{};
    std::uint64_t last_busy_snapshot_{};
    std::uint64_t last_plugin_snapshot_[kPluginLoadEntries]{};
