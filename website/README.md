...
手動滾輪／觸控可暫時接管示範進度，以輕微延遲平滑跟隨，停止 450 ms 後接續播放；倒回捲動同樣回到對應的中間狀態。每章示範在結果短暫停留後自動推進下一章，07 播畢循環回 01；任何手動滾動或章節導覽會永久交還控制，停在當前章的結果。八個視窗高度的捲動距離，加一個視窗高度的 sticky 場景；約一分鐘是閱讀目標。
...
選單、指標、外掛插入、連線繪製與 bypass 由同一個示範 phase 決定；訊號粒子與電平使用短循環。自動示範不產生聲音；展示被覆蓋、離開畫面或分頁進入背景時暫停。游標圓環表示整頁閱讀比例，下方細條與章節列當前項的填充同步表示該幕示範進度。手機使用原生觸控捲動、固定章節列及依可用高度縮放的操作區。尺寸切換時，尚未完成的導覽會優先到達原本指定的目的地。
