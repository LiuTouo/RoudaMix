<script lang="ts">
  ...
  const m = $derived(workflowMotion(progress));
...
<div bind:this={root} class="scene animated-workflow" class:static-view={staticView} class:started={m.stage > 0} data-stage={m.stage}
      ...
      <div class="mix-main">
        ...
        <div class="settings-card" class:highlight={m.stage === 1}>
          ...
          <div class="device-menu demonstration-menu" style={`opacity:${m.deviceMenu};transform:translateY(${(1 - m.deviceMenu) * -12}px) scale(${0.96 + m.deviceMenu * 0.04});visibility:${m.deviceMenu > 0 ? 'visible' : 'hidden'}`} aria-hidden="true">
            <span>ASIO</span><strong class:selected={m.deviceSelected > 0.1}>{zh ? '系統音訊 (WASAPI)' : 'System audio (WASAPI)'}<b>✓</b>{#if !staticView}<MotionPointer phase={m.devicePointer} label={zh ? '選擇裝置' : 'Select device'} />{/if}</strong>
          </div>
        </div>
        <div class="track" class:highlight={m.stage === 1} style={`--track-enter:${m.inputReveal}`}>
          ...
          {#if !staticView}<MotionPointer phase={m.inputPointer} label={zh ? '加入麥克風' : 'Add microphone'} />{/if}
        </div>
        <div class="plugin-slot" class:highlight={m.stage === 2} class:loaded={m.plugin}>
          ...
          <div class="plugin-picker-illustration demonstration-menu" style={`opacity:${m.pluginMenu};transform:translateY(${(1 - m.pluginMenu) * 20}px) scale(${0.94 + m.pluginMenu * 0.06});visibility:${m.pluginMenu > 0 ? 'visible' : 'hidden'}`} aria-hidden="true">
            <span class="tiny-label">{zh ? '外掛清單 / 已安裝' : 'Plugin list / Installed'}</span><div><strong>{zh ? '混響 / Reverb' : 'Reverb'}</strong><span>VST3</span><b>{zh ? '加入 →' : 'Add →'}</b>{#if !staticView}<MotionPointer phase={m.pluginPointer} label={zh ? '加入外掛' : 'Add plugin'} />{/if}</div>
          </div>
          {#if !staticView}<MotionPointer phase={m.bypassPointer} label={zh ? '啟用 Monitor Bypass' : 'Enable Monitor Bypass'} />{/if}
        </div>
        ...
        <div class="outputs" class:highlight={m.stage === 3 || m.stage === 4}>
          <div class="output monitor" style={`opacity:${0.3 + m.monitorRoute * 0.7};transform:translateY(${(1 - m.monitorRoute) * 8}px)`}>
            ...
            {#if !staticView}<MotionPointer phase={m.monitorPointer} label={zh ? '連到監聽' : 'Connect monitor'} />{/if}
          </div>
          <div class="output stream" style={`opacity:${0.3 + m.streamRoute * 0.7};transform:translateY(${(1 - m.streamRoute) * 8}px)`}>
            ...
            {#if !staticView}<MotionPointer phase={m.streamPointer} label={zh ? '連到串流' : 'Connect stream'} />{/if}
          </div>
        </div>
  ...
  <div class="destination-app" class:visible={m.obsReveal > 0} class:discord={m.voiceReveal > 0.5} style={`opacity:${m.obsReveal};transform:translate(${(1 - m.obsReveal) * 48 + Math.sin(m.voiceReveal * Math.PI) * 24}px,${(1 - m.obsReveal) * 20}px)`}>
    ...
    <div class="app-device-menu demonstration-menu" aria-hidden="true" style={`opacity:${m.obsMenu};transform:translateY(${(1 - m.obsMenu) * 12}px);visibility:${m.obsMenu > 0 ? 'visible' : 'hidden'}`}><span>{zh ? '裝置' : 'Device'}</span><strong>CABLE Output <b>✓</b>{#if !staticView}<MotionPointer phase={m.obsPointer} label={zh ? '選取輸入裝置' : 'Select input device'} />{/if}</strong></div>
    {#if !staticView}<MotionPointer phase={m.voicePointer} label={zh ? '選取 CABLE Output' : 'Select CABLE Output'} />{/if}
