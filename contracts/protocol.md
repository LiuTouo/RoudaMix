...
| `track_set_sidechain` | 側鏈路由:trackId 限 kind `fx`、sources 限 input 軌(kind `audio`/`app`);self → `bad_command`、未知 id → `track_not_found`;與 dests 合併成環 → `cycle_detected` 且**不套用**。訊號 = 來源軌 post-fader tap,經 PDC 對齊後只進 fx 軌 plugin 的 aux input(VST3 kAux bus),不進可聽混音、不進 monitor bus;plugin 無 aux bus 時餵靜音(路由照樣存在)。結果 = status `tracks[].sidechain` |
